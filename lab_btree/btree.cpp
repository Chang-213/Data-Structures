...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    auto ccbeg = child->children.begin();
    auto cebeg = child->elements.begin();
    auto ccend = child->children.end();
    auto ceend = child->elements.end();
    parent->children.insert(child_itr, new_right);
    parent->elements.insert(elem_itr, child->elements[mid_elem_idx]);
    new_right->elements.assign(mid_elem_itr + 1, ceend);
    new_left->elements.assign(cebeg, mid_elem_itr);
    if(child->is_leaf == false){
    new_right->children.assign(mid_child_itr, ccend);
    new_left->children.assign(ccbeg, mid_child_itr);
  }
}
